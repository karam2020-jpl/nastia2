...
try {
  ...
  assert.equal((await req('/api/admin/users',root,'POST',{username:'support',name:'الدعم',role:'support',active:true,password})).status,201);
  const support=await loginAs('support');
  for(const path of Object.values(endpoints))assert.equal((await req(path,support)).status,403,'support denied '+path);
  for(const role of ['orders','products','content'])assert.ok([401,403].includes((await req('/api/admin/support',cookies[role])).status));
  const publicPost=body=>fetch(base+'/api/support',{method:'POST',headers:{'Content-Type':'application/json',Origin:base},body:JSON.stringify(body)});
  const created=await publicPost({action:'create',name:'عميل',phone:'07701234567',order:'1',category:'شكوى',body:'اختبار الدعم'});assert.equal(created.status,201);const access=await created.json();
  assert.equal((await publicPost({action:'lookup',id:access.id,code:'0'.repeat(32)})).status,404);
  assert.equal((await req('/api/admin/support',support,'PATCH',{id:access.id,status:'قيد المعالجة',body:'تمت المتابعة'})).status,200);
  const tracked=await (await publicPost({action:'lookup',...access})).json();assert.equal(tracked.messages[0].body,'تمت المتابعة');assert.equal(tracked.access_hash,undefined);
  assert.equal((await publicPost({action:'reply',...access,body:'شكراً'})).status,200);
  assert.equal((await req('/api/admin/support',root)).status,200);
  console.log('PASS: support creation, secret access, staff isolation, owner access and replies');
  assert.equal((await req('/api/admin/categories',root,'POST',{name:'ملابس نسائية'})).status,201);
  assert.equal((await req('/api/admin/categories',root,'POST',{name:'ملابس نسائية'})).status,409);
  assert.equal((await req('/api/admin/categories',support,'POST',{name:'غير مسموح'})).status,403);
  let categoryCatalog=await (await fetch(base+'/api/catalog')).json();assert.ok(categoryCatalog.categories.includes('ملابس نسائية'));
  const categoryProduct={...categoryCatalog.products[0],id:'category-test',category:'ملابس نسائية'};
  assert.equal((await req('/api/admin/products',root,'POST',categoryProduct)).status,200);
  assert.equal((await req('/api/admin/categories',root,'DELETE',{name:'ملابس نسائية'})).status,409);
  const iconForm=()=>{const f=new FormData();f.set('name','ملابس نسائية');f.set('oldName','ملابس نسائية');return f;};
  let icon=iconForm();icon.set('image',new Blob([png],{type:'image/png'}),'icon.png');
  assert.equal((await fetch(base+'/api/admin/categories',{method:'PATCH',headers:{Cookie:root,Origin:base},body:icon})).status,200);
  let detail=await (await req('/api/admin/categories?details=1',root)).json();let iconUrl=detail.find(c=>c.name==='ملابس نسائية').image;assert.equal((await fetch(base+iconUrl)).status,200);
  icon=iconForm();icon.set('image',new Blob(['<svg/>'],{type:'image/png'}),'bad.png');assert.equal((await fetch(base+'/api/admin/categories',{method:'PATCH',headers:{Cookie:root,Origin:base},body:icon})).status,400);
  assert.equal((await req('/api/admin/categories',root,'PATCH',{oldName:'ملابس نسائية',name:'ملابس'})).status,200);
  categoryCatalog=await (await fetch(base+'/api/catalog')).json();assert.equal(categoryCatalog.products.find(p=>p.id==='category-test').category,'ملابس');
  assert.equal((await req('/api/admin/categories',root,'POST',{name:'قسم فارغ'})).status,201);
  assert.equal((await req('/api/admin/categories',root,'DELETE',{name:'قسم فارغ'})).status,200);
  detail=await (await req('/api/admin/categories?details=1',root)).json();iconUrl=detail.find(c=>c.name==='ملابس').image;assert.equal((await fetch(base+iconUrl)).status,200);
  assert.ok((await (await fetch(base)).text()).includes('/api/category-image?'));
  icon=new FormData();icon.set('name','ملابس');icon.set('oldName','ملابس');icon.set('removeImage','true');assert.equal((await fetch(base+'/api/admin/categories',{method:'PATCH',headers:{Cookie:root,Origin:base},body:icon})).status,200);
  assert.equal((await fetch(base+iconUrl)).status,404);
  console.log('PASS: category icon upload, invalid file rejection, rename preservation, homepage display and removal');
  console.log('PASS: dynamic category creation, rename cascade, deletion guard and owner restriction');
