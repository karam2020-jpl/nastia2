...
import {matchesOrder,orderDate} from '../admin-order-utils';
...
export default function AdminDashboard({user}:{user:AdminUser}){
  ...
  const filteredOrders=orders.filter(o=>matchesOrder(o,orderQuery));
  const load=async()=>{
    setLoading(true);try{
  ...
  return <div className="admin"><aside className="sidebar"><div className="logo">Nastia Beauty</div><p>{user.name}<br/><small>{roleLabels[user.role]}</small></p><nav>{allowed.map(([key,label])=><button disabled={busy} className={tab===key?'active':''} key={key} onClick={()=>setTab(key)}>{label}</button>)}<button onClick={logout}>تسجيل الخروج</button></nav></aside>
    <main className="adminmain"><div className="title-row"><h1>{tabs.find(t=>t[0]===tab)?.[1]}</h1>{tab==='products'&&<button className="btn" onClick={()=>setEditing(blank())}>+ إضافة منتج</button>}</div>
      ...
      {tab==='orders'&&<section className="panel"><label className="field admin-search">البحث عن العميل أو الطلب<input type="search" value={orderQuery} onChange={e=>setOrderQuery(e.target.value)} placeholder="اسم العميل، رقم الهاتف أو رقم الطلب"/></label><p className="muted" role="status">{filteredOrders.length} طلب — التوقيت المحلي: بغداد</p><table><thead><tr><th>الطلب</th><th>العميل</th><th>تاريخ الحجز</th><th>المحافظة</th><th>الإجمالي</th><th>الحالة</th><th/></tr></thead><tbody>{!filteredOrders.length&&<tr><td colSpan={7}>لا توجد طلبات مطابقة للبحث.</td></tr>}{filteredOrders.map(o=><tr key={o.id}><td>#{o.id}</td><td>{o.customer_name}<small style={{display:"block"}} dir="ltr">{o.phone}</small></td><td>{orderDate(o.created_at)}</td><td>{o.province}</td><td>{money(o.total)}</td><td><select disabled={busy} value={o.status} onChange={e=>{const status=e.target.value;void mutate(async()=>{await adminRequest('/api/admin/orders',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:o.id,status})});});}}>{statuses.map(s=><option key={s}>{s}</option>)}</select></td><td><button className="choice" onClick={()=>setSelected(o)}>التفاصيل</button></td></tr>)}</tbody></table></section>}
