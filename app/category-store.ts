...
export function ensureCategories(db:DatabaseSync,defaults:string[]){
 ...
 db.exec('CREATE TABLE IF NOT EXISTS category_image_preferences(name TEXT PRIMARY KEY COLLATE NOCASE REFERENCES categories(name) ON UPDATE CASCADE ON DELETE CASCADE)');
...
export function categoryDetails(db:DatabaseSync){return db.prepare('SELECT c.name,i.version,p.name AS customized FROM categories c LEFT JOIN category_images i ON c.name=i.name LEFT JOIN category_image_preferences p ON c.name=p.name ORDER BY c.rowid').all().map(r=>({name:String(r.name),customized:Boolean(r.customized)||Boolean(r.version),image:r.version?`/api/category-image?name=${encodeURIComponent(String(r.name))}&v=${r.version}`:''}));}
export function saveCategoryImage(db:DatabaseSync,name:string,image:{bytes:Uint8Array;mime:string}|null|undefined){if(image===undefined)return;db.prepare('INSERT OR IGNORE INTO category_image_preferences(name) VALUES(?)').run(name);if(image===null){db.prepare('DELETE FROM category_images WHERE name=?').run(name);return;}db.prepare('INSERT INTO category_images VALUES(?,?,?,?) ON CONFLICT(name) DO UPDATE SET bytes=excluded.bytes,mime=excluded.mime,version=excluded.version').run(name,image.bytes,image.mime,randomUUID());}
